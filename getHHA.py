# -*- coding: utf-8 -*-
"""
Created on Sun Feb 13 17:49:06 2022
...
'''
def getImage(root='H:/spyder/SLM'):
    D = cv2.imread(os.path.join(root, '0.tif'), cv2.COLOR_BGR2GRAY)
    # RD = cv2.imread(os.path.join(root, '0.tif'), cv2.COLOR_BGR2GRAY)
    return D
...
# genarate
def gen(dir_path1, dir_path2, C):
    path1 = dir_path1
    path2 = dir_path2
    img_name_list = os.listdir(path1)
    # print(img_name_list)
    
    for i in range(len(img_name_list)):
        D = cv2.imread(dir_path1 + '/' + img_name_list[i], cv2.COLOR_BGR2GRAY)
        
        print('processing img_{}'.format(i))
        print('max gray value: ', np.max(D))
        hha = getHHA(C, D, D)
        
        out_img_name = path2 + '/' + img_name_list[i]
        cv2.imwrite(out_img_name, hha)



def genHHA():
    camera_matrix = getCameraParam('color')
    labeled_path = 'F:/DFC22/labeled_train'
    sub_labeled_list = os.listdir(labeled_path)
    
    for sub_name in sub_labeled_list:
        sub_path = labeled_path + '/' + sub_name + '/' + 'RGEALTI'
        sub_out_path = labeled_path + '/' + sub_name + '/' + 'HHA'
        os.makedirs(sub_out_path, exist_ok=True)
        gen(sub_path, sub_out_path, camera_matrix)

    unlabeled_path = 'F:/DFC22/unlabeled_train'
    sub_unlabeled_list = os.listdir(unlabeled_path)
    
    for sub_name in sub_unlabeled_list:
        sub_path = unlabeled_path + '/' + sub_name + '/' + 'RGEALTI'
        sub_out_path = unlabeled_path + '/' + sub_name + '/' + 'HHA'
        os.makedirs(sub_out_path, exist_ok=True)
        gen(sub_path, sub_out_path, camera_matrix)
    
    val_path = 'F:/DFC22/val'
    sub_val_list = os.listdir(val_path)
    
    for sub_name in sub_val_list:
        sub_path = val_path + '/' + sub_name + '/' + 'RGEALTI'
        sub_out_path = val_path + '/' + sub_name + '/' + 'HHA'
        os.makedirs(sub_out_path, exist_ok=True)
        gen(sub_path, sub_out_path, camera_matrix)
...
if __name__ == "__main__":
    ...
    # print(hha.shape) # (1000, 1000, 3)
    cv2.imwrite('hha.tif', hha)
